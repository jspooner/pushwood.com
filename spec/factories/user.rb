FactoryGirl.define do
  ...
  factory :user_photo, :parent => :user do |user|
    after(:create) { |u| u.roles << FactoryGirl.single_instance(:role_photo) }
  end
  factory :user_no_photo, :parent => :user do |user|
    after(:create) { |u| u.roles.delete_if { |r| role.name == "photo" } }
  end
  factory :user_admin, :parent => :user do |user|
    sequence(:email) {|n| "admin.#{n}@gmail.com" }
    after(:create) { |u| u.roles << FactoryGirl.single_instance(:role_admin) }
  end
